...
def most_spoken_languages(
    countries: list[dict[str, Any]], top_n: int = 10
) -> list[tuple[str, int]]:
    """Find the most commonly spoken languages across a dataset of countries.

    Args:
        countries: A list of country dicts, each expected to have a
            "languages" key mapping to a list of language names.
        top_n: The number of top languages to return

    Returns:
        A list of (language, count) tuples, ordered by frequency descending.
    """
    language_occurrence = []

    for country in countries:
        if country.get("languages"):
            language_occurrence.extend(country["languages"])

    return Counter(language_occurrence).most_common(top_n)


most_spoken_languages(countries, top_n=10)
...
def most_populated_countries(
    ...
    """Find the most populated countries across a dataset of countries.

    Args:
        countries: A list of country dicts, each expected to have "name"
            and "population" keys.
        top_n: The number of top countries to return.

    Returns:
        A list of (country, population) tuples, ordered by population descending.
    """
    country_populations = []

    for country in countries:
        if country.get("population") and country.get("name"):
            country_populations.append((country["name"], country["population"]))

    return sorted(country_populations, key=lambda pair: pair[1], reverse=True)[:top_n]


most_populated_countries(countries, top_n=10)
